...
// Rate limiting for username changes - once per 30 minutes
const usernameChangeAttempts = new Map<string, number>();
const USERNAME_CHANGE_COOLDOWN = 30 * 60 * 1000; // 30 minutes

function canChangeUsername(userId: string): boolean {
  const lastChange = usernameChangeAttempts.get(userId);
  if (!lastChange) return true;
  return Date.now() - lastChange >= USERNAME_CHANGE_COOLDOWN;
}

function recordUsernameChange(userId: string): void {
  usernameChangeAttempts.set(userId, Date.now());
}
...
router.post('/register', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user.id, username: user.username, tokenVersion: 0 },
...
router.post('/update-username', async (req, res) => {
  try {
    ...
    // Check rate limit - can only change username once per 30 minutes
    if (!canChangeUsername(decoded.userId)) {
      const lastChange = usernameChangeAttempts.get(decoded.userId) || 0;
      const minutesLeft = Math.ceil((USERNAME_CHANGE_COOLDOWN - (Date.now() - lastChange)) / 60000);
      return res.status(429).json({ error: `Username can only be changed once per 30 minutes. Please wait ${minutesLeft} minutes.` });
    }
    ...
    io.emit('user-updated', { userId: user.id, username: user.username });
    
    // Record successful username change for rate limiting
    recordUsernameChange(user.id);
...
// Update theme colors
router.post('/update-theme', authenticate, async (req, res) => {
  try {
...
router.get('/user/:username', async (req, res) => {
  try {
    ...
    const currentUserFriendIds = new Set([
      ...currentUserFriends.map((f: any) => f.userId === currentUserId ? f.friendId : f.userId)
    ]);

    const mutualFriends = userFriends
      .map((f: any) => f.userId === user.id ? f.friend : f.user)
      .filter((f: any) => currentUserFriendIds.has(f.id));

    // Get mutual groups (groups that both users are in)
    const userGroups = await prisma.room.findMany({
      where: {
        type: 'GROUP',
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    let decoded;
    try {
      decoded = jwt.verify(token, process.env.JWT_SECRET || 'secret') as { userId: string };
    } catch {
    ...
    io.emit('force-logout', { userId: decoded.userId });
